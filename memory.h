...
template<typename T>
T RPM(uintptr_t address,HANDLE hGame) {
    T buffer{};
...
template<typename T>
bool RPM(uintptr_t address, T& outBuffer, HANDLE hGame) {
    SIZE_T bytesRead = 0;
...
template<>
inline float* RPM<float*>(uintptr_t address, HANDLE hGame) {
...
template<typename T>
void RPM(uintptr_t address, T* outBuffer, size_t count, HANDLE hGame) {
    ReadProcessMemory(hGame, reinterpret_cast<LPCVOID>(address), outBuffer, sizeof(T) * count, nullptr);
}

template<typename T>
void WPM(uintptr_t address, const T& value, HANDLE hGame) {
    WriteProcessMemory(hGame, reinterpret_cast<LPVOID>(address), &value, sizeof(T), nullptr);
}

inline void WPM(uintptr_t address, const BYTE* buffer, size_t size, HANDLE hGame) {
